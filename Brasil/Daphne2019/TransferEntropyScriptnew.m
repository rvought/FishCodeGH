...
[TE_fish_cave,TE_fish_srf,TE_fish_cave_std,TE_fish_srf_std] = deal([]);
[dist_fish_cave,dist_fish_srf,dist_fish_cave_std,dist_fish_srf_std] = deal([]);
    ...
    if nFish>2
        ...
        for f = 1:nFish
            ...
            if j<=nCave
                ...
                dist_fish_cave = [dist_fish_cave;nanmean(dist_all{j}(:,f))];
                dist_fish_cave_std = [dist_fish_cave_std;nanstd(dist_all{j}(:,f))];                
            else
                TE_fish_srf = [TE_fish_srf;nanmean(TEmax(idx,:))];
                TE_fish_srf_std = [TE_fish_srf_std;nanstd(TEmax(idx,:))];
                
                dist_fish_srf = [dist_fish_srf;nanmean(dist_all{j}(:,f))];
                dist_fish_srf_std = [dist_fish_srf_std;nanstd(dist_all{j}(:,f))];
            end
        end
    end
end
...
for k = 1:nChannelPairs
    ...
    figure(13); subplot(nChannelPairs,1,k); hold on;
    plot(meanSRFdist, TE_pair_srf(:,k), 'bo');
    plot(meanCAVEdist, TE_pair_cave(:,k), 'ro'); % esfesf
...
%% PLOT TE versus distance
meanCAVEdist = []; stdCAVEdist = []; 
meanSRFdist = []; stdSRFdist = []; 
...
for j = 1:(nCave+nSrf)
    ...
    if nFish>2
        ...
        for p = 1:nPairs
            ...
            if j<=nCave
                meanCAVEdist(end+1) = nanmean(dist_all{j}(:,p));
                stdCAVEdist(end+1) = nanstd(dist_all{j}(:,p));
            else
                meanSRFdist(end+1) = nanmean(dist_all{j}(:,p));
                stdSRFdist(end+1) = nanstd(dist_all{j}(:,p));
            end
        end
    end
end
...
%% sample plotter
% j = 2727; % set to some number (1 to 51364)
figure(1);clf;
subplot(211); 
yyaxis left; plot(data.time{j}, data.trial{j}(1,:)/max(data.trial{j}(1,:)));
    hold on;
yyaxis right; plot(data.time{j}, data.trial{j}(2,:)/max(data.trial{j}(2,:))); 
xlim([0 data.time{j}(end)])
subplot(212); plot(TE(:,1,j)); xlim([0 length(TE(:,1,j))]); 
ylim([-0.2 0.2]);
j=j+1;
