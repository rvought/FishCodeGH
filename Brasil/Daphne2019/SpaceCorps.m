...
% Bounding box fitted to the spatial extent of data
...
% THE CRITICAL PARAMETERS (sort of...) for calculating the histograms
...
for j = numfish:-1:1 % For each fish    
    ...
    out(j).xbounds = [xminedge xmaxedge];
    out(j).ybounds = [yminedge ymaxedge];
    ...
    % tr is the data within our time range
        tr = find(in.fish(feesh(j)).freq(:,1) > tims(1) & in.fish(feesh(j)).freq(:,1) < tims(2));
    ...
    %% Generate corresponding jiggled and random fish
        ...
        % Both random and jiggled fish have a jiggled initial theta
            ...
            deltatheta = constrainer * (rand(1,1) - 0.5); 
            % Set a random change in direction for our artificial fish
    ...
    for rr = 2:length(out(j).valididx)
            ...
            if out(j).rndXY(1,end) > xmaxedge
                out(j).rndXY(1,end) = xmaxedge - (out(j).rndXY(1,end) - xmaxedge);
            end
            if out(j).rndXY(2,end) < yminedge
                out(j).rndXY(2,end) = yminedge + (yminedge - out(j).rndXY(2,end));
            end
            if out(j).rndXY(2,end) > ymaxedge
                out(j).rndXY(2,end) = ymaxedge - (out(j).rndXY(2,end) - ymaxedge);
            end
            ...
            if out(j).jigXY(1,end) > xmaxedge
                out(j).jigXY(1,end) = xmaxedge - (out(j).jigXY(1,end) - xmaxedge);
            end
            if out(j).jigXY(2,end) < yminedge
                out(j).jigXY(2,end) = yminedge + (yminedge - out(j).jigXY(2,end));
            end
            if out(j).jigXY(2,end) > ymaxedge
                out(j).jigXY(2,end) = ymaxedge - (out(j).jigXY(2,end) - ymaxedge);
            end
