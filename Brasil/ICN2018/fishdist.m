...
for j=1:length(in) % For each entry
    ...
    for ff = 1:length(in(j).c) % for each fish
        ...
        out(j).fish(ff).mfreq = mean(in(j).c(ff).freq(cts));
        out(j).fish(ff).vfreq = var(in(j).c(ff).freq(cts));
        out(j).fish(ff).Nfreq = length(cts);

        % Velocity and distance of the fish
        
        for vv = 2:length(cts)
            X(1,1) = in(j).c(ff).tx(cts(vv-1));
            X(2,1) = in(j).c(ff).ty(cts(vv-1));
            X(1,2) = in(j).c(ff).tx(cts(vv));
            X(2,2) = in(j).c(ff).ty(cts(vv));
            dist(vv) = pdist(X);
            vel(vv) = dist(vv) / (in(j).c(ff).tt(cts(vv)) - in(j).c(ff).tt(cts(vv-1)));
        end
