...
tim = 1/Fs:1/Fs:length(sig)/Fs; % Time

for idx = length(spikes):-1:1
    if spikes(idx) > wid && spikes(idx) < tim(end)-wid
        temp = interp1(tim, sig, spikes(idx)-wid:1/Fs:spikes(idx)+wid);
        sta(idx,:) = temp;
    end
end
for idx = length(randspikes):-1:1
    if randspikes(idx) > wid && randspikes(idx) < tim(end)-wid
