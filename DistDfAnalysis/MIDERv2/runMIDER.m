...
%--------------------------------------------------------------------------
function [Output,options] = runMIDER(x,variables,options)
...
if ~isfield(options,'useStatistics')
    options.useStatistics = 1;
end

% Data curation step:
if ~isfield(options,'correctOutliers')
