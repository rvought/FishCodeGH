...
accsteps = 0.0000015;
posedges = -5:0.1:5;
veledges = min(out.vel):velsteps:max(out.vel); % need to define bins rather than calculate them on a per trial basis 

accedges = min(out.acc):accsteps:max(out.acc); % need to define bins rather than calculate them on a per trial basis 
